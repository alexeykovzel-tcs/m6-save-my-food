...
class ReceiptScanner {
  ...
  static Future<List<Product>?> _apiScan(XFile receipt) async {
    ...
    return _findProducts(
      lines,
      ['OMSCHRIJVING'],
      ['TOTAAL', 'SUBTOTAAL', 'BETALEN', 'ANTAL'],
    );
  }
  ...
  static List<Product>? _findProducts(
    ...
    for (String line in lines.sublist(startIdx + 1, endIdx)) {
      ...
      bool hasQuantity = RegExp('^[0-9]*\$').hasMatch(fields[0]);
      String productName = hasQuantity ? fields[1] : fields[0];
      if (_containsAny(productName, [',', '+', '='])) continue;

      // Add product with expiration date from 2 to 9
      productName = _capitalize(productName);
      int daysAgo = Random().nextInt(8) + 2;
      Product product = Product.byDaysAgo(productName, daysAgo: daysAgo);
